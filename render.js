function drawSpacetimeGrid() {
  ...
  let baseCol = getThemeColor(0.4, activeThemeIndex);
  stroke(red(baseCol), green(baseCol), blue(baseCol), 18);
  strokeWeight(0.85);
  noFill();
  ...
  for (let i = 0; i < numCircles; i++) {
    ...
    for (let j = 0; j <= numSegments; j++) {
      ...
      if (dist2D === 0) continue; // Защита от NaN
...
function drawSmoothParticle(sx, sy, psx, psy, val, opacity) {
  ...
  let col = getThemeColor(localVal, activeThemeIndex);
...
function drawCellContent(style, c, r, centerX, centerY, col, val, cell) {
  ...
  if (style === 0) {
    ...
    text(asciiStr.charAt(charIdx), centerX, centerY);
  } else if (style === 2) {
    // Плавный матричный глиф с меньшим мерцанием
    let matrixChars = "ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇ0123456789ABCDEF";
    let seed = Math.floor((c * 37 + r * 17 + Math.floor(frameCount * 0.1)) % matrixChars.length);
    textAlign(CENTER, CENTER);
    textSize(cellSize * 1.25);
    text(matrixChars.charAt(seed), centerX, centerY);
  }
}