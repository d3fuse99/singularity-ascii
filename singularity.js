...
class Particle {
  ...
  update() {
    ...
    // Смягченный и плавный эффект увлечения пространства (без резких скачков)
...
function draw() {
  ...
  // Рисуем звезды с эффектом деформации в дуги Эйнштейна
  for (let s of stars) {
    ...
    if (dist2D > 0) {
      ...
      if (finalX >= 0 && finalX < width && finalY >= 0 && finalY < height) {
        ...
        if (sz > 0 && stretchRatio > 1.05) {
          ...
          stroke(s.brightness, s.brightness, s.brightness + 15, 180);
          strokeWeight(s.size);
