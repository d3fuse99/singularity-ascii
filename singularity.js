...
function setup() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    pg.noStroke();
    let col = spriteColors[i];
    for (let r = 32; r > 0; r -= 1) {
      let alpha = map(r, 0, 32, 180, 0);
      pg.fill(red(col), green(col), blue(col), alpha * 0.08);
      pg.ellipse(32, 32, r * 2, r * 2);
    }
    gasSprites.push(pg);
  }
...
class Particle {
  ...
  update() {
    ...
    this.angle += this.speed * timeDilation;
  }
}
...
function draw() {
  ...
  for (let i = 0; i < activeCount; i++) {
    ...
    if (dist2D > 0) {
      ...
      if (sz > 0) {
        let lensedDist1 = (dist2D + Math.sqrt(dist2D * dist2D + 4 * RE * RE)) / 2;
        let k = map(sz, 0, 30, 0, 1, true) * Math.abs(Math.cos(pitch)); 
  ...
  if (drawSmooth) {
    for (let i = 0; i < numParticles; i++) {
      ...
      if (pp.active && pp.sz > 0 && pp.dFromCenter >= R_shadow) {
        drawSmoothParticle(pp.sx, pp.sy, pp.intensity, smoothOpacity);
      }
    }
...
function drawSmoothParticle(sx, sy, val, opacity) {
  let dFromCenter = Math.sqrt(sx * sx + sy * sy);
  let redshift = map(dFromCenter, R_shadow, R_shadow * 1.35, 1.0, 0.0, true);
  let localVal = lerp(val, val * 0.15, redshift);
  ...
  let sprite = gasSprites[spriteIdx];
  
  let baseSize = map(localVal, 0, 1, 6, 24);
  let stretchFactor = map(dFromCenter, R_shadow, maxRadius, 2.0, 1.0, true);
  let w = baseSize * stretchFactor;
  let h = baseSize / (stretchFactor * 0.7);
  ...
  image(sprite, sx + width / 2, sy + height / 2, w * 2.5, h * 2.5);
}
