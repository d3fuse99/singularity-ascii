...
function windowResized() {
  ...
  for (let p of particles) {
    ...
    p.y = (Math.random() - 0.5) * (p.r * 0.04 + 1.5);
  }
}
...
class Particle {
  constructor(r, angle) {
    ...
    this.y = (Math.random() - 0.5) * (r * 0.04 + 1.5);
  }
...
function draw() {
  ...
  for (let p of particles) {
    ...
    let n = noise(p.r * 0.03, p.angle * 2 + frameCount * 0.01);
    x3d += map(n, 0, 1, -2, 2);
    ...
    if (dist2D > 0) {
      let k = 1 / (1 + Math.exp(-sz * 0.15));
      
      let intensity = map(p.r, minRadius, maxRadius, 1.0, 0.12);
      intensity = constrain(intensity, 0, 1);
      
      let distFromScreenCenter1 = dist2D;
      let maxRenderRadius = Math.min(width, height) / 2 - 10;
      let boundaryFade = 1.0;
      let fadeStart = maxRenderRadius - 80;
      
      let lensedDist1 = (dist2D + Math.sqrt(dist2D * dist2D + 4 * RE * RE)) / 2;
      let finalDist1 = lerp(dist2D, lensedDist1, constrain(k, 0, 1));
      ...
      let secondaryFade = map(sz, 0, 45, 0.0, 1.0);
      secondaryFade = constrain(secondaryFade, 0, 1);
      
      if (secondaryFade > 0) {
        let lensedDist2 = (Math.sqrt(dist2D * dist2D + 4 * RE * RE) - dist2D) / 2;
        
        let sx2 = - (sx / dist2D) * lensedDist2;
        let sy2 = - (sy / dist2D) * lensedDist2;
        let dFromCenter2 = Math.sqrt(sx2 * sx2 + sy2 * sy2);
        
        if (dFromCenter2 >= R_shadow) {
          addToGrid(sx2, sy2, intensity * 0.45 * secondaryFade, sz);
        }
      }
    }
  }
    ...
    for (let r = 0; r < rows; r++) {
      ...
      if (cell.count > 0) {
        ...
        if (val < 0.25) {
          ...
          col = lerpColor(color(255, 130, 0), color(255, 220, 70), map(val, 0.55, 0.8, 0, 1));
        } else {
          col = lerpColor(color(255, 220, 70), color(255, 255, 255), map(val, 0.8, 1.0, 0, 1));
        }
        
        if (transitionProgress < 1.0) {
          drawCell(currentStyle, c, r, centerX, centerY, col, val, cell, (1 - transitionProgress) * 255);
          drawCell(nextStyle, c, r, centerX, centerY, col, val, cell, transitionProgress * 255);
        } else {
          drawCell(currentStyle, c, r, centerX, centerY, col, val, cell, 255);
        }
      }
    }
  }
...
function drawCell(style, c, r, centerX, centerY, col, val, cell, opacity) {
  let colWithAlpha = color(col);
  colWithAlpha.setAlpha(opacity);
  ...
  if (style === 0) {
    ...
    let matrixCol = lerpColor(color(5, 190, 45), color(230, 255, 235), val);
    if (Math.random() < 0.06) matrixCol = color(255, 90, 0);
    
    let matrixColWithAlpha = color(matrixCol);
    matrixColWithAlpha.setAlpha(opacity);
    
    fill(matrixColWithAlpha);
    textAlign(CENTER, CENTER);
